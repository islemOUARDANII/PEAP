...
export default function AdvisorCandidateToOffersMatching() {
    ...
    const navCandidateId = (location.state as { candidateId?: string; candidateName?: string } | null)?.candidateId ?? '';
    const navCandidateName = (location.state as { candidateId?: string; candidateName?: string } | null)?.candidateName ?? '';

    const [query, setQuery] = useState('');
    const [submittedQuery, setSubmittedQuery] = useState('');
    const [selectedCandidateId, setSelectedCandidateId] = useState(navCandidateId);
    ...
    const launchMutation = useMutation({
        mutationFn: async () => {
            ...
            const run = await gatewayApi.matching.createRun({
                ...
                source_entity_id: selectedCandidateId,
                parameters_json: {
                    min_score: 0,
                },
            });
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
                <div className="space-y-4">
                    {navCandidateId && (
                    <div className="rounded-xl border border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950/30 p-3 flex items-center gap-2 text-sm text-green-700 dark:text-green-400">
                        <CheckCircle className="h-4 w-4 shrink-0" />
                        <span>
                            Candidat pré-sélectionné
                            {navCandidateName ? ` : ${navCandidateName}` : ''}
                        </span>
                    </div>
                )}
