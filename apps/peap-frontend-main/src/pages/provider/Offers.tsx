    ...
    <div className="space-y-6">
      ...
      <div className="panel overflow-hidden">
        ...
        {!isLoading && !isError && filteredJobs.length === 0 && (
          <div className="px-4 py-4 text-sm text-muted-foreground">No offers match the selected filters.</div>
        )}
        {!isLoading && !isError && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-muted text-xs text-muted-foreground">
                  ...
                  <th className="px-2 py-3 text-right font-medium">Applicants</th>
              ...
              <tbody className="divide-y divide-border">
                {filteredJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-surface-muted">
                    ...
                    <td className="px-2 py-3 align-top text-xs text-muted-foreground">{job.location}</td>
                    <td className="px-2 py-3 align-top text-xs text-foreground">{job.contract}</td>
                    <td className="px-2 py-3 align-top text-right text-sm font-mono text-foreground">{job.matched}</td>
                    <td className="px-2 py-3 align-top text-right text-sm font-mono text-muted-foreground">{job.applicants}</td>
