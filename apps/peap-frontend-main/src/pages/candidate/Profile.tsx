...
async function syncDraftCollection<TCurrent extends { id: string }, TDraft extends { id?: string }>(
  ...
  const keptIds = new Set(
    draftItems.map((item) => item.id).filter((value): value is string => Boolean(value)),
  );
...
export default function Profile() {
  ...
  const delegationsQuery = useQuery({
    ...
    queryFn: () => gatewayApi.referentials.delegations(draft?.governorateCode || undefined),
  ...
  const waitForCvParsed = async (cvId: string): Promise<CandidateCvRecord> => {
    ...
    const successStatuses = new Set(["PARSED", "READY", "COMPLETED", "AVAILABLE"]);
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      const currentParsingStatus = (currentCv.parsingStatus ?? "").toUpperCase();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Unable to save the profile");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <article className="panel-elevated overflow-hidden">
        <div className="border-b border-border bg-gradient-to-br from-primary/[0.06] via-background to-accent-soft/50 px-6 py-7 sm:px-8">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <InfoCard icon={MapPin} label="Location" value={displayLocation || "Not set"} />
            <InfoCard icon={Mail} label="Email" value={bundle?.contact?.email || "Not set"} />
            <InfoCard icon={Globe2} label="Primary language" value={bundle?.primaryLanguage || "Not set"} />
            <InfoCard icon={Languages} label="Languages" value={String(bundle?.languages.length ?? 0)} />
          </div>
        </div>
        ...
        <div className="space-y-8 px-6 py-7 sm:px-8">
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, firstName: value } : current)
                }
                disabled={!isEditing}
              />
              <Field
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, lastName: value } : current)
                }
                disabled={!isEditing}
              />
              <Field
              ...
              <Field
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, nationality: value } : current)
                }
                disabled={!isEditing}
              />
              <Field
              ...
              <Field
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, primaryLanguage: value } : current)
                }
                disabled={!isEditing}
              />
              <OptionSelect
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, genderCode: value } : current)
                }
                  ...
                  setDraft((current) =>
                    current
                      ? {
                        ...
                        delegationCode: "",
                        preferredGovernorate: current.preferredGovernorate || value,
                      }
                      : current,
                  )
                }
              ...
              <OptionSelect
                ...
                onChange={(value) =>
                  setDraft((current) => current ? { ...current, delegationCode: value } : current)
                }
              ...
              <div className="grid gap-4 md:grid-cols-2 md:col-span-2">
                <Field
                  ...
                  onChange={(value) =>
                    setDraft((current) => current ? { ...current, cin: value } : current)
                  }
                  disabled={!isEditing}
                />
                <Field
                  ...
                  onChange={(value) =>
                    setDraft((current) => current ? { ...current, passportNumber: value } : current)
                  }
                  disabled={!isEditing}
                />
              </div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <OptionSelect
                ...
                onChange={(value) =>
                  setDraft((current) =>
                    current ? { ...current, preferredContractType: value } : current
                  )
                }
              ...
              <OptionSelect
                ...
                onChange={(value) =>
                  setDraft((current) =>
                    current ? { ...current, preferredGovernorate: value } : current
                  )
                }
                options={governoratesQuery.data ?? []}
                placeholder="Select governorate"
              ...
              <label className="flex items-center gap-2 text-sm text-foreground">
                <input
                  ...
                  onChange={(event) =>
                    setDraft((current) =>
                      current
                        ? { ...current, acceptsRelocation: event.target.checked }
                        : current
                    )
                  }
                />
          ...
          <CollectionSection<EducationDraft>
            ...
            renderItem={(item, update) => (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Level code" value={item.levelCode} onChange={(value) => update({ ...item, levelCode: value })} disabled={!isEditing} />
                <Field label="Diploma" value={item.diplomaLabel} onChange={(value) => update({ ...item, diplomaLabel: value })} disabled={!isEditing} />
                <Field label="Specialty" value={item.specialty} onChange={(value) => update({ ...item, specialty: value })} disabled={!isEditing} />
                <Field label="Institution" value={item.institution} onChange={(value) => update({ ...item, institution: value })} disabled={!isEditing} />
                <Field label="Graduation year" type="number" value={item.graduationYear} onChange={(value) => update({ ...item, graduationYear: value })} disabled={!isEditing} />
              </div>
            )}
            viewItem={(item) => (
              <>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {[item.institution, item.specialty, item.graduationYear].filter(Boolean).join(" - ")}
                </p>
              </>
            )}
          />
          ...
          <CollectionSection<ExperienceDraft>
            ...
            renderItem={(item, update) => (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Job title" value={item.jobTitleRaw} onChange={(value) => update({ ...item, jobTitleRaw: value })} disabled={!isEditing} />
                <Field label="Company" value={item.companyName} onChange={(value) => update({ ...item, companyName: value })} disabled={!isEditing} />
                <Field label="Sector" value={item.sector} onChange={(value) => update({ ...item, sector: value })} disabled={!isEditing} />
                <Field label="Start date" type="date" value={item.startDate} onChange={(value) => update({ ...item, startDate: value })} disabled={!isEditing} />
                <Field label="End date" type="date" value={item.endDate} onChange={(value) => update({ ...item, endDate: value })} disabled={!isEditing} />
          ...
          <CollectionSection<SkillDraft>
            ...
            renderItem={(item, update) => (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Skill" value={item.skillLabelRaw} onChange={(value) => update({ ...item, skillLabelRaw: value })} disabled={!isEditing} />
                <Field label="Level" value={item.level} onChange={(value) => update({ ...item, level: value })} disabled={!isEditing} />
                <Field label="Years" type="number" value={item.years} onChange={(value) => update({ ...item, years: value })} disabled={!isEditing} />
                <Field label="Evidence" value={item.evidence} onChange={(value) => update({ ...item, evidence: value })} disabled={!isEditing} />
              </div>
            )}
            viewItem={(item) => (
              <div className="flex flex-wrap gap-2">
                <SkillTag label={item.skillLabelRaw || "Skill"} variant="matched" />
          ...
          <CollectionSection<LanguageDraft>
            ...
            renderItem={(item, update) => (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Language code" value={item.languageCode} onChange={(value) => update({ ...item, languageCode: value })} disabled={!isEditing} />
                <Field label="Level" value={item.level} onChange={(value) => update({ ...item, level: value })} disabled={!isEditing} />
                <Field label="Evidence" value={item.evidence} onChange={(value) => update({ ...item, evidence: value })} disabled={!isEditing} />
              </div>
            )}
            viewItem={(item) => (
              <div className="flex flex-wrap gap-2">
                <SkillTag label={inferLanguageLabel({ id: "", languageCode: item.languageCode, level: item.level, evidence: item.evidence, createdAt: "", updatedAt: "" })} />
              </div>
            )}
          />
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-border bg-background p-5">
                <p className="text-sm font-semibold text-foreground">Competences detectees</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {bundle?.skills.length ? (
                    bundle.skills.map((skill) => (
                      <SkillTag key={skill.id} label={inferSkillLabel(skill)} variant="matched" />
                    ))
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      Aucune competence n'est encore enregistree. Importez un CV ou ajoutez-les manuellement.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-border bg-background p-5">
                <p className="text-sm font-semibold text-foreground">Competences numeriques</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {codingSkillLabels.length ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      Les competences techniques apparaitront ici lorsqu'elles seront detectees dans votre profil.
                    </p>
                  )}
                </div>
              </div>
...
function CollectionSection<T extends { id?: string }>({
  ...
  icon: ComponentType<{ className?: string }>;
  items: T[];
  setItems: Dispatch<SetStateAction<T[]>> | ((next: SetStateAction<T[]>) => void);
  emptyItem: () => T;
  isEditing: boolean;
    ...
    <section className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        {items.map((item, index) => (
          <article key={item.id ?? `${title}-${index}`} className="rounded-2xl border border-border bg-background p-5">
            {isEditing ? (
              <div className="space-y-4">
