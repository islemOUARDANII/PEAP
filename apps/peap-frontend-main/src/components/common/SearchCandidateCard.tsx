...
function matchTone(score: number) {
  if (score >= 80) return { label: 'Fort', tone: 'success' as const };
  if (score >= 50) return { label: 'Modere', tone: 'warning' as const };
  return { label: 'Faible', tone: 'destructive' as const };
}
    ...
    <article key={candidate.id} className="panel p-5 flex flex-col relative">
      {isTop && (
        <span className="absolute -top-2 left-4 inline-flex items-center gap-1 rounded-full bg-warning text-warning-foreground px-2 py-0.5 text-[10px] font-semibold">
          <Trophy className="h-3 w-3" /> Meilleure correspondance
        </span>
      )}
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5 text-[11px] text-muted-foreground">
              ...
              <span>·</span>
              <span>{candidate.experienceYears} ans d'exp.</span>
            </div>
          </div>
        </div>
        {candidate.score && (
      ...
      <div className="rounded-md bg-surface-muted border border-border px-2.5 py-1.5 mb-3 flex items-center justify-between text-xs">
        <span className="flex items-center gap-1.5 text-success">
          <CheckCircle2 className="h-3 w-3" /> {matched?.length}/
          {required?.length} competences requises
        </span>
        {missing?.length > 0 && (
          <span className="flex items-center gap-1.5 text-destructive">
            <XCircle className="h-3 w-3" /> Manquantes :{' '}
      ...
      <div className="mt-auto pt-3 border-t border-border flex gap-1.5">
        ...
        >
          ...
          />
          {isShort ? 'Preselectionne' : 'Preselectionner'}
        </Button>
