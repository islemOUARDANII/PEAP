...
const PARSE_POLL_TIMEOUT_MS = 120_000;

const pollParsingStatus = async (cvRecordId: string): Promise<string> => {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const cvRecords = await gatewayApi.candidate.listCvRecords();
      const cv = cvRecords.find((r) => r.id === cvRecordId);
      const status = cv?.parsingStatus ?? 'UNKNOWN';
...
export const uploadAndParseCandidateCv = async (
  ...
  options?: {
    ...
    onParsed?: (status: string) => void | Promise<void>;
  },
): Promise<{
  ...
  const immediateResult = await gatewayApi.candidate.parseCv(record.id);

  if (immediateResult.parsingStatus !== 'PARSING') {
    return { record, parseResult: immediateResult };
  }

  const finalStatus = await pollParsingStatus(record.id);

  if (options?.onParsed) {
    await options.onParsed(finalStatus);
  }
