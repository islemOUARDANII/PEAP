...
@router.post("/advisor/candidates", response_model=AdvisorCreateCandidateResponse, status_code=201)
def advisor_create_candidate(
    payload: AdvisorCreateCandidateRequest,
    db: Session = Depends(get_db),
...
@router.get("/advisor/employers", response_model=list[EmployerListItemResponse])
def advisor_list_employers(
    db: Session = Depends(get_db),
    _current_user=Depends(require_roles("ANETI_ADVISOR", "FUNCTIONAL_ADMIN")),
):
...
@router.post("/advisor/offers", response_model=dict, status_code=201)
def advisor_create_offer(
    payload: AdvisorCreateOfferRequest,
    db: Session = Depends(get_db),
    current_user=Depends(require_roles("ANETI_ADVISOR", "FUNCTIONAL_ADMIN")),
):
    return create_offer(db, payload, current_user)
